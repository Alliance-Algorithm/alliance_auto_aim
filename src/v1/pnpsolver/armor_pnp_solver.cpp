...
public:
    static std::optional<const  world_exe::data::ArmorCameraSpacing> Solve(const world_exe::data::ArmorImageSpacing& armors) {

        cv::Mat rvec, tvec;
        auto& objectPoints =
            armors.isLargeArmor ? LargeArmorObjectPoints : NormalArmorObjectPoints;
        if (cv::solvePnP(
                objectPoints, armors.image_points, util::Profile::get_intrinsic_parameters(),
...
const  world_exe::interfaces::IArmorInCamera&  ArmorPnPSolver::SolvePnp(const world_exe::interfaces::IArmorInImage& armors) {
...
const world_exe::interfaces::ITimeStamped& ArmorPnPSolver::GetTimeStamped()  const {
    return *this;
}

const std::time_t& ArmorPnPSolver::GetTimeStamp()const  {
    return time_point_;
}

const std::vector<world_exe::data::ArmorCameraSpacing>& ArmorPnPSolver::GetArmors(
